...
def hash_to_semantic_polarity(genesis_hash: str) -> str:
    ...
    return f"51% HUMANO - 49% MAQUINA - FOLIO:{FOLIO} - {genesis_hash[:8]} - 51%_HUMANO"
...
def generate_manifesto_from_hash(genesis_hash: str, pct_humano: int = 51, pct_maquina: int = 49) -> str:
    ...
    return f"FOLIO:{FOLIO}|GENESIS:{genesis_hash}|MANIFIESTO 51/49: {pct_humano}% HUMANO - {pct_maquina}% MAQUINA - 51%_HUMANO - 49%_MAQUINA - DETERMINISTICO-{hash_short}-{suffix}"

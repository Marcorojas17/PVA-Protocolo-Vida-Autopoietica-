...
def generar_pdf(folio_cliente, op_id, email_cliente):
    folio_safe = "".join(c for c in folio_cliente if c.isalnum() or c in "-_")[:50]
    ...
    c.drawString(50,h-160,f"Hash: {HASH_GENESIS[:32]}...")
...
def enviar_correo(destinatario, pdf_path, folio_cliente):
    try:
        ...
        msg.attach(MIMEText("Gracias por tu compra. Adjunto dictamen.","plain"))
...
def validar_firma(req):
    ...
    if not MP_WEBHOOK_SECRET or not x_sig: return False
    try:
...
def mp_webhook():
    if not validar_firma(request): return jsonify({"error":"Invalid"}),401

...
def test_folio_no_duplicado_en_contrato():
    ...
    """
    registros_simulados = {
        FOLIO: {"folio": FOLIO, "hashGenesis": GENESIS}
    }
    # Intentar registrar mismo folio debe fallar
    folio_a_registrar = FOLIO
    ya_existe = folio_a_registrar in registros_simulados and len(registros_simulados[folio_a_registrar]["folio"]) > 0
    assert ya_existe is True, "Folio 5204160405358537 ya debe existir - contrato debe rechazar duplicado"
...
def test_sello_ligado_a_tx():
    ...
    assert verify_on_etherscan(registro_onchain["tx"], {"status": "1", "result": {}}) is True
